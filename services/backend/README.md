...
Memory edits, soft deletes, consolidation, and retention set a durable `vector_sync_pending` intent in the same Postgres statement as the source-of-truth change. Reconciliation reads the current row to activate or deactivate deterministic and legacy Weaviate objects, then clears the marker only after success; failures remain retryable and recall continues to enforce Postgres `is_active` as the authority. Memory fact UUIDs are the deterministic Weaviate object IDs for new writes, while legacy objects are discovered by `pgFactId`. Consolidation also compares the pre-LLM `updated_at` versions before applying an action, so an edit made during the model round trip is not superseded from stale input.
...
`POST /documents/extract` treats Docling and Tika as untrusted upstream boundaries. A Docling `200` response must contain string `content`, a non-empty string `format`, object `metadata`, and an optional list of object-valued `chunks`; malformed success payloads fail instead of being indexed as empty documents. Upstream response bodies and transport exceptions are written only to Backend logs. The public route returns the stable `Document extraction failed` error so provider details and document content cannot cross the API boundary.
...
When `FAL_SOURCE=enabled`, `POST /media/generate` accepts a provider-neutral request with `provider`, `modality`, `model`, and `input` fields (plus optional `consumer`/`project` attribution). The registry supports `provider=fal` with `modality=image` and `modality=image_to_3d`, and `provider=comfyui` with `modality=image` (#519 — backed by the managed/localhost ComfyUI host via `COMFYUI_ENDPOINT`; local generation records `cost_usd=0.0` for provenance and serves artifacts through the backend `/comfyui/image/{filename}` proxy); unsupported provider/modality pairs return `400` before any provider client is initialized. For `provider=comfyui`, `model` accepts **either** a curated catalog entry name (e.g. `krea2-turbo-bf16`, the identity configured in `COMFYUI_USER_MODELS`) — resolved via the ComfyUI manifest (`COMFYUI_MANIFEST_PATH`) to the bundle's per-role diffusion/text-encoder/VAE filenames (#675) — **or** a raw checkpoint/diffusion filename; when the manifest is available an unknown token returns `400` naming both accepted forms, and when it is absent the token passes through to ComfyUI verbatim. For `modality=image`, `input.prompt` must be a non-empty string of at most 4,000 characters, `seed` must be a JSON integer when present, and `image_size` must be an object when present. The default `fal-ai/flux/dev` text endpoint accepts width and height integers from 64 through 4096, 1–50 steps, CFG 1–20, and 1–4 images. Supplying `image_url`, `image`, or `init_image` selects `fal-ai/flux/dev/image-to-image`, which accepts 10–50 steps, CFG 1–20, 1–4 images, and optional finite `strength` from 0.01 through 1; that endpoint does not accept output dimensions. Neither default FLUX endpoint accepts `negative_prompt`, so Atlas rejects it instead of claiming the provider applied it. A custom image endpoint requires `input.provider_arguments` containing the exact provider-native payload, including a prompt matching `input.prompt`; Atlas forwards that object without inventing a cross-model schema. Image-to-3D accepts only `input.image` plus an optional JSON-integer `seed`; Atlas emits the selected endpoint's exact field (`image_url`, `input_image_url`, or Rodin's one-element `input_image_urls`) and rejects generic `extra` overrides. Only the verified TRELLIS, Hunyuan3D, Tripo, and Rodin registry entries are advertised or routable; Pixal3D remains deferred until its endpoint contract is verified. Optional mapped values default only when absent or null, and invalid or endpoint-incompatible values return `400` before operation-store, budget, storage, or provider work. The legacy `POST /comfyui/generate` FAL compatibility path supports only `fal-ai/flux/dev`, an empty negative prompt, and the mapped text-to-image controls; custom endpoints use `/media/generate`. `FAL_TIMEOUT_SECONDS` must be finite, greater than zero, and at most 3,600; `FAL_OUTPUT_FORMAT` must be `jpeg` or `png`; and malformed safety-checker booleans fail Backend startup. `POST /media/generate` returns `202` with an operation id and `GET /media/operations/{operation_id}` polls the same selected provider endpoint into a normalized response containing status, provider, model, modality, artifacts, cost, license, and provenance. **`artifact_url` contract (#678):** the URL shape is **provider-dependent** — for `provider=fal` it is an **absolute** hosted CDN URL (e.g. `https://cdn.example/result.png`), while for `provider=comfyui` it is a **gateway-relative** backend proxy path (e.g. `/comfyui/image/atlas_00001_.png?folder_type=output`), the same in-network `GET /comfyui/image/{filename}` proxy Open WebUI/n8n use — deliberately relative because the backend cannot know each consumer's reachable base (in-network `backend:8000`, the Kong gateway, or a public ingress). A consumer MUST therefore **resolve a relative `artifact_url` against its own gateway/backend base URL** before fetching it (a leading `/` — no `http(s)://` scheme — signals the relative form); fetching a relative URL verbatim fails with `missing 'http://' protocol`. The structured `artifacts[]` entries additionally carry `filename` / `subfolder` / `folder_type` for callers that reconstruct the URL themselves. The `GET /comfyui/image/{filename}` proxy validates those query params before contacting ComfyUI (#801): `folder_type` must be one of `output` / `input` / `temp`, and a `subfolder` containing `..`, a leading `/`, or a NUL byte is rejected with `400`. `POST /media/operations/{operation_id}/cancel` records nonterminal `cancellation_requested` and asks FAL to cancel (`provenance.provider_cancellation_requested`); the reservation remains held until a later provider poll confirms terminal `cancelled`, `failed`, or `succeeded`, preventing accepted-but-still-running work from being silently uncharged. Unknown ids return `404`; terminal operations and repeated cancellation requests return `409`. Poll, timeout, and cancellation writes compare the previously observed state before updating it, so a stale poll cannot replace a concurrent cancellation request. When a poll transitions an op to `timeout`, Atlas best-effort cancels the underlying provider job (ComfyUI dequeue/interrupt; FAL cancel) so a timed-out op no longer orphans a running GPU/cloud workload (#676); that cancel is logged, never raised, and does not change the timeout outcome or the ledger reconciliation. Set the request's top-level `timeout_seconds` above the provider's cold-start worst case — a cold Krea 2 BF16 load on the managed-MPS host is ~90–120 s before the first sampler step — or an otherwise-successful generation is timed out (and cancelled) mid-flight. Before paid submission, the route verifies that shared operation state is reachable. A post-submission persistence failure is retried three times, then Atlas attempts provider cancellation and returns `503` with the provider operation id, whether the provider accepted the cancellation request, and `manual_reconciliation_required=true`. Provider acceptance is not terminal proof, so every unpersisted paid operation retains its budget reservation until an operator reconciles it. Every blocking FAL SDK submit, status, result, and cancel call is bounded by `FAL_TIMEOUT_SECONDS`; timeout failures are surfaced through the operation contract rather than occupying an API worker indefinitely. Request bodies over `MEDIA_REQUEST_MAX_BYTES` are rejected with `413` before route parsing, including streamed bodies without `Content-Length`. Inline images are bounded by `MEDIA_INPUT_MAX_BYTES` before base64 decoding and by `MEDIA_INPUT_MAX_PIXELS` before Pillow conversion or conditioned-canvas allocation. `FAL_API_KEY` remains backend-only and is never returned in API responses.
...
**RAG ingestion job engine (`rag_ingestion/`, #413):** `POST /api/rag/ingestions` runs a generic, repeatable ingestion lifecycle over a consumer-declared `rag_ingestion_profile` — discover → parse (Docling → Tika → plain-text fallback) → chunk (Chonkie) → embed (LiteLLM) → vector-store write (Weaviate, class namespaced `{collection_prefix}_{profile}`) → LightRAG upload → drain (poll extraction with a timeout) → finalize. Parser-order entries invoke the named extractor exactly; unsupported or unavailable parsers advance to the next entry. Each phase records status, counts, timing, and actionable per-file errors, and every target is **capability-gated** by its backend's endpoint env var with per-target `on_unavailable: fail | skip` semantics. Jobs are **idempotent** by consumer + profile revision + a real corpus fingerprint and run through the Celery tier when enabled (else synchronously in-request); synchronous discovery and Chonkie work are moved off the API event loop. State is held in the ingestion store (Redis when `REDIS_URL` is set, else in-memory), and Redis operations use bounded connect/read deadlines. Idempotency claims, cancellation, terminal transitions, and dispatch-failure recovery are atomic in that store: concurrent submissions share one job, cancellation cannot be erased by a stale worker save, and a broker failure releases the key for a fresh retry. Every execution also acquires an owner-fenced lease (`RAG_INGESTION_EXECUTION_LEASE_SECONDS`, 10–300 seconds, default 30) before any phase side effect, renews it while running, and may persist only while it still owns that lease. Late duplicate deliveries therefore cannot overwrite a replacement worker; a busy Celery delivery waits for lease expiry and retries. In worker mode, connection and timeout failures leave the current phase pending and propagate to Celery's bounded backoff/retry policy; synchronous fallback keeps the established terminal-record response contract. **Drain is resilient to transient status-poll failures (#673):** LightRAG can briefly stop servicing `GET /documents/pipeline_status` during a long extraction/merge, so the drain loop retries `httpx` timeout/transport errors (e.g. `ReadTimeout`) within the profile's `timeout_seconds` drain deadline using bounded exponential backoff with full jitter, rather than failing an otherwise-healthy ingestion. Cancellation and the execution-lease heartbeat stay responsive across retries. Deterministic HTTP errors (401/403, validation, other 4xx) are **never** retried — they fail immediately with bounded, actionable detail (status + truncated body). The drain phase records `status_polls` and `transient_retries` as evidence, and a terminal timeout names the last poll error's exception class even when its message is empty (the `ReadTimeout`-has-blank-`str()` trap). Operator knobs: the profile `timeout_seconds` sets the overall drain deadline; the per-request status-poll timeout is `LIGHTRAG_PIPELINE_STATUS_TIMEOUT_SECONDS` (default 30 s, read from the backend process environment). The profiles themselves are declared in `atlas.consumer.yml` and compiled by the bootstrapper into `RAG_INGESTION_PROFILES_FILE`; corpus inputs are a consumer-mounted read-only path under `RAG_INGESTION_CORPUS_ROOT` (default `/app/corpus`) or a MinIO prefix — never an arbitrary host path. Discovery reads in bounded chunks and rejects inputs over `RAG_INGESTION_MAX_FILE_BYTES` (100 MiB per file), `RAG_INGESTION_MAX_CORPUS_BYTES` (1 GiB aggregate), or `RAG_INGESTION_MAX_FILES` (10,000 files) before retaining unbounded content in memory. Consumer-side walkthrough: [reusing-atlas.md §6.3.4](../../docs/deployment/reusing-atlas.md#634-declaring-rag-ingestion-profiles-with-rag_ingestion_profiles). Covered by `app/app/tests/test_rag_ingestion.py` + `test_rag_ingestion_api.py` (fake upstreams; a live round-trip is an optional test).

Submission persists the effective corpus and normalized profile snapshot with the job, so a queued worker executes the submitted definition even if the generated registry changes before delivery. Weaviate create `422` responses count as replay only after Atlas confirms that the deterministic object or class exists; unrelated schema, property, and vector validation failures remain visible. After writing the desired generation, Atlas removes prior objects for the same profile that are no longer present, including trailing chunks from shortened documents and every chunk from removed sources.

The Backend API and Celery worker share the same Redis state database, compiled profile registry, upstream endpoints, resource limits, and scoped MinIO credential references. Lease renewal failure cancels the active async phase and reschedules the ingestion after the lease window; the final transient attempt records a terminal failed job instead of leaving a deduplicating pending record. Celery tracks earlier transient connection and timeout failures with a dedicated three-retry exponential-backoff counter, so repeated lease-contention retries cannot consume the failure-recovery budget. LightRAG uploads use deterministic content-and-path identities; replayed uploads treat the pinned server's duplicate-source `409` as idempotent success without colliding equal basenames from different corpus paths.
...
**Auth & errors.** The route requires `Authorization: Bearer <LIGHTRAG_RERANK_ADAPTER_TOKEN>` (constant-time compared): missing/invalid → `401`, token unset (adapter not configured) → `503`. Input is bounded (query/document sizes and count); an empty document list short-circuits to `{"results": []}` without calling TEI. TEI results require unique integer indexes, no more items than the submitted document count, and finite numeric scores; malformed, duplicate, excessive, or out-of-range items return `502` instead of being coerced. Other TEI errors surface as `502` (upstream error / malformed body), `504` (timeout), or `503` (TEI unreachable / not configured). Enabling reranking trades a little latency (an extra cross-encoder pass over retrieved chunks) for better passage ordering; leave it off if latency-sensitive.
