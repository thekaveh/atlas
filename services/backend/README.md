...
**Auth & errors.** The route requires `Authorization: Bearer <LIGHTRAG_RERANK_ADAPTER_TOKEN>` (constant-time compared): missing/invalid → `401`, token unset (adapter not configured) → `503`. Input is bounded (query/document sizes and count); an empty document list short-circuits to `{"results": []}` without calling TEI. TEI results require integer indexes and finite numeric scores; malformed or out-of-range items return `502` instead of being coerced. Other TEI errors surface as `502` (upstream error / malformed body), `504` (timeout), or `503` (TEI unreachable / not configured). Enabling reranking trades a little latency (an extra cross-encoder pass over retrieved chunks) for better passage ordering; leave it off if latency-sensitive.
