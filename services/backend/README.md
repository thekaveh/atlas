...
Lease renewal failure cancels the active async phase before the stale worker exits. Celery also tracks transient connection and timeout failures with a dedicated three-retry exponential-backoff counter, so repeated lease-contention retries cannot consume the failure-recovery budget.
