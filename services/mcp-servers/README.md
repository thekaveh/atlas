...
| Tenant-scoped Postgres reads | partial | tested | MCP uses a dedicated read-only login without object ownership or BYPASSRLS and permits bounded SELECT/WITH/SHOW/EXPLAIN; it still has broad pg_read_all_data access with no schema, table, or column allowlist or redaction, so access remains operator-scoped rather than tenant-scoped. |
