...
Atlas does not run a FAL container. The backend reads `FAL_SOURCE`, `FAL_API_KEY`, and model defaults from the environment, then routes compatible simple generation requests to the fal.ai Python client when `FAL_SOURCE=enabled`.
...
| FAL provider | `FAL_SOURCE=enabled` | Enables FAL-backed simple generation through the backend compatibility route. |
| Backend route | `POST /comfyui/generate` | Uses FAL when `FAL_SOURCE=enabled`; otherwise preserves the existing ComfyUI path. |
...
| `FAL_MODEL` | `fal-ai/flux/dev` | FAL model endpoint used for text-to-image generation. |
| `FAL_TIMEOUT_SECONDS` | `120` | Backend timeout budget for synchronous generation calls. |
...
- Backend integration: `POST /comfyui/generate` chooses FAL first when `FAL_SOURCE=enabled`, keeping existing Open WebUI and n8n callers compatible.
...
- Dedicated `/media/generate` route that makes provider selection explicit while keeping `/comfyui/generate` as a compatibility surface.
...
- FAL queue webhooks are not wired in this first provider pass. The backend uses synchronous generation for the existing request/response route.
