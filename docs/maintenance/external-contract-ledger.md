...
| n8n community package install endpoint | `n8nio/n8n:2.28.2` | Official n8n docs expose `POST /api/v1/community-packages` with `X-N8N-API-KEY`, but Atlas first-boot init has no UI-issued API key yet | Atlas pins the image-internal `/rest/community-packages` first-boot route for install/check calls until setup can provision a public API key; init exits nonzero on required community-node failures and startup waits for the one-shot exit | Guarded by `bootstrapper/tests/test_shell_script_contracts.py` and `bootstrapper/tests/test_start_post_launch_verification.py` |
