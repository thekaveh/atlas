...
| AIRFLOW_DB_PASSWORD | airflow | atlas-db-password | Auto-generated by bootstrapper. Postgres role for the Airflow metadata DB. |
| AIRFLOW_DB_USER_URI | airflow | airflow | Bootstrapper-synchronized percent-encoded AIRFLOW_DB_USER for DSN userinfo. |
| AIRFLOW_DB_PASSWORD_URI | airflow | atlas-db-password | Bootstrapper-synchronized percent-encoded AIRFLOW_DB_PASSWORD for DSN userinfo. |
| AIRFLOW_ATLAS_DB_USER | airflow | atlas_airflow_reader | Read-only login seeded into the postgres_supabase Airflow Connection. |
| AIRFLOW_ATLAS_DB_PASSWORD | airflow | atlas-db-password | Auto-generated password for Airflow DAGs' read-only Atlas database connection. |
...
| BACKEND_DB_USER | backend | atlas_backend | Scoped PostgreSQL login for Backend and its Celery worker. |
| BACKEND_DB_PASSWORD | backend | atlas-db-password | Auto-generated password for the scoped Backend database role. |
| BACKEND_DB_USER_URI | backend | atlas_backend | Bootstrapper-synchronized percent-encoded BACKEND_DB_USER for DSN userinfo. |
| BACKEND_DB_PASSWORD_URI | backend | atlas-db-password | Bootstrapper-synchronized percent-encoded BACKEND_DB_PASSWORD for DSN userinfo. |
...
| CHONKIE_SEMANTIC_EMBEDDING_MODEL | backend | minishlab/potion-base-32M | Non-empty Chonkie embedding model reference used by semantic chunking. Blank values use the bundled default. Shared by Backend and Celery ingestion workers. |
| LIGHTRAG_PIPELINE_STATUS_TIMEOUT_SECONDS | backend | 30 | Finite per-request timeout in seconds for LightRAG pipeline-status polling. Values must be greater than 0 and no greater than 3600; invalid values use 30 seconds. Shared by Backend and Celery ingestion workers. |
| RAG_INGESTION_TTL_SECONDS | backend | 604800 | Redis retention in seconds for RAG ingestion records and idempotency keys. Values must be from 60 through 31536000 (one year); invalid values use the seven-day default. Shared by Backend and Celery ingestion workers. |
...
| BACKEND_STATE_STORE_MODE | backend | redis | Shared operation-state mode: 'redis' (durable, multi-process default) or 'memory' (explicit single-process testing/ephemeral mode). Redis outages fail stateful routes with HTTP 503; they never trigger an in-memory fallback. |
...
| MEDIA_BUDGET_ENABLED | backend | False | Enables media spend accounting + budget enforcement (#342). When off, ordinary operations record no spend rows and enforce no caps; ambiguous FAL submissions still create a recovery row. |
| MEDIA_BUDGET_STORE | backend | postgres | Ledger/recovery store: 'postgres' (durable, media_spend_ledger table) or 'memory' (process-local, non-durable — testing/ephemeral). Ambiguous FAL submissions use it for recovery even when budget enforcement is disabled. |
...
| LANGMEM_EMBEDDING_MODEL | backend |  | Optional LiteLLM embedding model id. Empty falls back to LITELLM_EMBEDDING_MODEL. |
| LANGMEM_EMBEDDING_DIM | backend | 768 | Validated output dimension shared by the selected LangMem embedding model, Backend pgvector queries, and the Supabase memory schema migration. Curated wizard selections derive this from services/*/models.yaml; custom models must declare it explicitly (1..4000, the pgvector halfvec HNSW limit). |
...
| BACKUP_S3_MODE | backup | local | Credential/dependency mode: local uses on-stack MinIO and permits the documented MINIO_ROOT_* upgrade fallback; external requires dedicated BACKUP_S3_* credentials and has no MinIO runtime dependency. |
| BACKUP_S3_ENDPOINT | backup | http://minio:9000 | S3 endpoint as a strict HTTP(S) origin: canonical DNS/IPv4 or bracketed IPv6, optional canonical port, and no userinfo/path/query/fragment. The default is on-stack MinIO. |
| BACKUP_S3_ACCESS_KEY | backup |  | Dedicated S3 access key. Required for external endpoints; local MinIO may fall back to MINIO_ROOT_USER for upgrade compatibility. |
| BACKUP_S3_SECRET_KEY | backup |  | Dedicated S3 secret key. Required for external endpoints; local MinIO may fall back to MINIO_ROOT_PASSWORD for upgrade compatibility. |
| BACKUP_S3_REGION | backup | us-east-1 | S3 signing and bucket-creation region. |
| BACKUP_S3_SESSION_TOKEN | backup |  | Optional S3 security token for temporary credentials. |
| BACKUP_S3_TLS_VERIFY | backup | True | Verify HTTPS certificates. false is valid only with an https endpoint and explicitly enables the MinIO client's insecure certificate mode. |
| BACKUP_S3_ALIAS_URL | backup |  | Deprecated pre-M11 endpoint variable accepted only as a migration fallback when BACKUP_S3_ENDPOINT remains at its local default. |
...
| BACKUP_RESTORE_GLOBAL_TIMEOUT_SECONDS | backup | 28800 | Finite active-restore deadline; must exceed BACKUP_COMMAND_TIMEOUT_SECONDS. Hard-kill grace covers three bounded commands plus 60 seconds for compensation and exact lock cleanup. |
| BACKUP_MANIFEST_HMAC_KEY | backup |  | Required operator-supplied 64-hex HMAC-SHA-256 key for trusted backup manifests; preserve it outside the backup bucket for disaster recovery. |
| BACKUP_DEPLOYMENT_ID | backup |  | Required stable operator-supplied deployment identity bound into every trusted backup manifest. |
| BACKUP_MAX_POSTGRES_DUMP_BYTES | backup | 10737418240 | Finite maximum PostgreSQL dump size accepted by both backup production and bounded restore download. |
| BACKUP_RESTORE_MAX_CANDIDATES | backup | 100 | Maximum newest completion markers inspected by latest restore; use an exact BACKUP_TIMESTAMP for older backups. |
| BACKUP_DATABASES | backup | True | Capture Neo4j and Weaviate using their consistency-safe contracts. Run through services/backup/run-consistent-backup.sh so Neo4j is quiesced first. |
| BACKUP_DATABASE_QUIESCE_TIMEOUT_SECONDS | backup | 120 | Bounded host-side stop, start, and database snapshot status deadline. |
| BACKUP_MAX_DATABASE_ARCHIVE_BYTES | backup | 53687091200 | Maximum bytes accepted for each Neo4j or Weaviate snapshot archive during backup and restore. |
| BACKUP_LOCAL_SNAPSHOT_RETENTION_COUNT | backup | 3 | Completed local Neo4j/Weaviate native snapshot sets retained after authenticated publication (1-100); pruning runs only while the database services are quiesced. |
| BACKUP_LOCAL_ROLLBACK_RETENTION_COUNT | backup | 1 | Validated offline rollback volumes retained per database after a successful restore cutover (1-20). |
...
| BOOTSTRAPPER_PORT_LAYOUT_VERSION | globals | 5 | Sentinel updated by bootstrapper/services/migrations. Missing or <N triggers the chained env migrations up to N (v1 port-layout, v2 URL>PORT, v3 COMFYUI model-set, v4 stale curated Ollama model cleanup, v5 Weaviate backup-filesystem enablement). Fresh .env files are already v5-shaped. |
...
| ICEBERG_DB_PASSWORD | iceberg-rest | atlas-db-password | Auto-generated by bootstrapper. Postgres role for the Iceberg JDBC catalog DB. |
...
| ICEBERG_REST_POSTGRES_JDBC_SHA512 | iceberg-rest | 3759e7160591863e5100361298943df94488a6e4ee03936d20723638142fe8038c9a6a47fa8ee7e424f4ef09bb351edc89dfe2ae4acdfc0f92699a8b00196c5c | SHA-512 digest required for the PostgreSQL JDBC driver selected above. Update both values together. |
...
| JUPYTER_DB_PASSWORD_URI | jupyterhub | atlas-db-password | Bootstrapper-synchronized percent-encoded JUPYTER_DB_PASSWORD for DSN userinfo. |
...
| LABEL_STUDIO_DB_NAME_URI | label-studio | label_studio | Bootstrapper-synchronized percent-encoded LABEL_STUDIO_DB_NAME for DSN path components. |
| LABEL_STUDIO_DB_USER | label-studio | label_studio | - |
| LABEL_STUDIO_DB_PASSWORD | label-studio | atlas-db-password | Auto-generated Postgres password for the dedicated Label Studio database role. |
...
| LANGFUSE_DB_NAME_URI | langfuse | langfuse | Bootstrapper-synchronized percent-encoded LANGFUSE_DB_NAME for DSN path components. |
| LANGFUSE_DB_USER | langfuse | langfuse | - |
| LANGFUSE_DB_PASSWORD | langfuse | atlas-db-password | Auto-generated password for the owner of Langfuse's dedicated database. |
| LANGFUSE_DB_USER_URI | langfuse | langfuse | Bootstrapper-synchronized percent-encoded LANGFUSE_DB_USER for DSN userinfo. |
| LANGFUSE_DB_PASSWORD_URI | langfuse | atlas-db-password | Bootstrapper-synchronized percent-encoded LANGFUSE_DB_PASSWORD for DSN userinfo. |
...
| LIGHTRAG_DB_USER | lightrag | atlas_lightrag | Scoped PostgreSQL owner for LightRAG PGVectorStorage relations. |
| LIGHTRAG_DB_PASSWORD | lightrag | atlas-db-password | Auto-generated password for the scoped LightRAG database role. |
| LIGHTRAG_DB_USER_URI | lightrag | atlas_lightrag | Bootstrapper-synchronized percent-encoded LIGHTRAG_DB_USER for DSN userinfo. |
| LIGHTRAG_DB_PASSWORD_URI | lightrag | atlas-db-password | Bootstrapper-synchronized percent-encoded LIGHTRAG_DB_PASSWORD for DSN userinfo. |
...
| LITELLM_CACHE_TTL | litellm | 3600 | Lifetime in seconds of a cached LiteLLM response. The gateway caches completions/embeddings in Redis; LiteLLM's own fallback is 60s, which is too short to pay off on anything but rapid-fire identical prompts. Raise for heavier reuse, lower if stale answers matter more than the saving. A non-numeric or non-positive value falls back to 3600 rather than failing the render. |
| LITELLM_CACHE_NAMESPACE | litellm | litellm.cache | Key prefix for LiteLLM's Redis response cache. Redis db 0 is shared with n8n's BullMQ queue, Kong's rate-limit counters, Langfuse's queue and the backend's media store, so a namespace keeps gateway cache keys scannable and separately droppable. |
| LITELLM_DB_NAME | litellm | litellm | - |
| LITELLM_DB_NAME_URI | litellm | litellm | Bootstrapper-synchronized percent-encoded LITELLM_DB_NAME for DSN path components. |
| LITELLM_DB_USER | litellm | litellm | - |
| LITELLM_DB_PASSWORD | litellm | atlas-db-password | Auto-generated password for the owner of LiteLLM's dedicated database. |
| LITELLM_DB_USER_URI | litellm | litellm | Bootstrapper-synchronized percent-encoded LITELLM_DB_USER for DSN userinfo. |
| LITELLM_DB_PASSWORD_URI | litellm | atlas-db-password | Bootstrapper-synchronized percent-encoded LITELLM_DB_PASSWORD for DSN userinfo. |
...
| MCP_POSTGRES_DB_USER | mcp-servers | atlas_mcp | Read-only PostgreSQL login for the curated MCP query tool. |
| MCP_POSTGRES_DB_PASSWORD | mcp-servers | atlas-db-password | Auto-generated password for the MCP read-only database role. |
...
| MLFLOW_DB_NAME_URI | mlflow | mlflow | Bootstrapper-synchronized percent-encoded MLFLOW_DB_NAME for DSN path components. |
| MLFLOW_DB_USER | mlflow | mlflow | - |
| MLFLOW_DB_PASSWORD | mlflow | atlas-db-password | Auto-generated Postgres password for the dedicated MLflow database role. |
| MLFLOW_DB_USER_URI | mlflow | mlflow | Bootstrapper-synchronized percent-encoded MLFLOW_DB_USER for DSN userinfo. |
| MLFLOW_DB_PASSWORD_URI | mlflow | atlas-db-password | Bootstrapper-synchronized percent-encoded MLFLOW_DB_PASSWORD for DSN userinfo. |
| N8N_SOURCE | n8n | container | - |
| N8N_DB_USER | n8n | atlas_n8n | Owner of the isolated n8n schema in Supabase PostgreSQL. |
| N8N_DB_PASSWORD | n8n | atlas-db-password | Auto-generated password for the scoped n8n database role. |
| N8N_PORT | n8n |  | - |
...
| OPEN_WEBUI_DB_USER | open-webui | atlas_open_webui | Scoped PostgreSQL owner for Open WebUI migration objects. |
| OPEN_WEBUI_DB_PASSWORD | open-webui | atlas-db-password | Auto-generated password for the scoped Open WebUI database role. |
| OPEN_WEBUI_DB_USER_URI | open-webui | atlas_open_webui | Bootstrapper-synchronized percent-encoded OPEN_WEBUI_DB_USER for DSN userinfo. |
| OPEN_WEBUI_DB_PASSWORD_URI | open-webui | atlas-db-password | Bootstrapper-synchronized percent-encoded OPEN_WEBUI_DB_PASSWORD for DSN userinfo. |
...
| SUPABASE_DB_NAME_URI | supabase | postgres | Bootstrapper-synchronized percent-encoded SUPABASE_DB_NAME for DSN path components. |
...
| SUPABASE_AUTH_DB_USER | supabase | supabase_auth_admin | - |
| SUPABASE_AUTH_DB_PASSWORD | supabase | atlas-db-password | Auto-generated login password for the upstream auth schema owner. |
| SUPABASE_AUTH_DB_USER_URI | supabase | supabase_auth_admin | Bootstrapper-synchronized percent-encoded SUPABASE_AUTH_DB_USER for DSN userinfo. |
| SUPABASE_AUTH_DB_PASSWORD_URI | supabase | atlas-db-password | Bootstrapper-synchronized percent-encoded SUPABASE_AUTH_DB_PASSWORD for DSN userinfo. |
| SUPABASE_STORAGE_DB_USER | supabase | supabase_storage_admin | - |
| SUPABASE_STORAGE_DB_PASSWORD | supabase | atlas-db-password | Auto-generated login password for the upstream storage schema owner. |
| SUPABASE_STORAGE_DB_USER_URI | supabase | supabase_storage_admin | Bootstrapper-synchronized percent-encoded SUPABASE_STORAGE_DB_USER for DSN userinfo. |
| SUPABASE_STORAGE_DB_PASSWORD_URI | supabase | atlas-db-password | Bootstrapper-synchronized percent-encoded SUPABASE_STORAGE_DB_PASSWORD for DSN userinfo. |
| SUPABASE_API_DB_USER | supabase | authenticator | - |
| SUPABASE_API_DB_PASSWORD | supabase | atlas-db-password | Auto-generated login password for PostgREST's role-switching authenticator. |
| SUPABASE_API_DB_USER_URI | supabase | authenticator | Bootstrapper-synchronized percent-encoded SUPABASE_API_DB_USER for DSN userinfo. |
| SUPABASE_API_DB_PASSWORD_URI | supabase | atlas-db-password | Bootstrapper-synchronized percent-encoded SUPABASE_API_DB_PASSWORD for DSN userinfo. |
| SUPABASE_REALTIME_DB_USER | supabase | atlas_realtime | - |
...
| POSTGRES_EXPORTER_DB_PASSWORD | supabase | atlas-db-password | Auto-generated password for the pg_monitor-only exporter role. |
| POSTGRES_EXPORTER_DB_USER_URI | supabase | atlas_metrics | Bootstrapper-synchronized percent-encoded POSTGRES_EXPORTER_DB_USER for DSN userinfo. |
| POSTGRES_EXPORTER_DB_PASSWORD_URI | supabase | atlas-db-password | Bootstrapper-synchronized percent-encoded POSTGRES_EXPORTER_DB_PASSWORD for DSN userinfo. |
...
| SUPAVISOR_DB_ADMIN_USER | supavisor | atlas_supavisor | Scoped owner for Supavisor's dedicated management database. |
| SUPAVISOR_DB_ADMIN_PASSWORD | supavisor | atlas-db-password | Auto-generated password for Supavisor management migrations and auth lookup. |
| SUPAVISOR_DB_ADMIN_USER_URI | supavisor | atlas_supavisor | Bootstrapper-synchronized percent-encoded SUPAVISOR_DB_ADMIN_USER for DSN userinfo. |
| SUPAVISOR_DB_ADMIN_PASSWORD_URI | supavisor | atlas-db-password | Bootstrapper-synchronized percent-encoded SUPAVISOR_DB_ADMIN_PASSWORD for DSN userinfo. |
...
| SUPAVISOR_TENANT_ID | supavisor | atlas | Supavisor tenant suffix. Clients connect as <db_user>.<tenant>; only ASCII letters, digits, underscores, and hyphens are accepted because dot is the upstream user/tenant delimiter. |
...
| TRINO_SCALE | trino |  | - |
| TRUEFORGE_SOURCE | trueforge | disabled | - |
| TRUEFORGE_PORT | trueforge |  | Host port for the TrueForge server UI + API (in-container 8790). |
| TRUEFORGE_ENDPOINT | trueforge |  | Resolved per TRUEFORGE_SOURCE — `http://trueforge:8790` for container, empty for disabled. |
| TRUEFORGE_API_KEY | trueforge |  | Auto-generated by the bootstrapper on first run. Service credential for the controller's internal schedule-dispatch calls to the server. |
| TRUEFORGE_DB_NAME | trueforge | trueforge | - |
| TRUEFORGE_DB_NAME_URI | trueforge | trueforge | Bootstrapper-synchronized percent-encoded TRUEFORGE_DB_NAME for DSN path components. |
| TRUEFORGE_DB_USER | trueforge | trueforge | - |
| TRUEFORGE_DB_PASSWORD | trueforge | atlas-db-password | Auto-generated password for the owner of TrueForge's dedicated database. |
| TRUEFORGE_DB_USER_URI | trueforge | trueforge | Bootstrapper-synchronized percent-encoded TRUEFORGE_DB_USER for DSN userinfo. |
| TRUEFORGE_DB_PASSWORD_URI | trueforge | atlas-db-password | Bootstrapper-synchronized percent-encoded TRUEFORGE_DB_PASSWORD for DSN userinfo. |
| TRUEFORGE_SCALE | trueforge |  | - |
| TRUEFORGE_CONTROLLER_SCALE | trueforge |  | - |
| TRUEFORGE_INIT_SCALE | trueforge |  | - |
...
| WEAVIATE_ENABLE_MODULES | weaviate | text2vec-openai,text2vec-ollama,multi2vec-clip,generative-openai,generative-ollama,backup-filesystem | - |
