...
- **Backend pins** (`services/backend/app/app/requirements.txt`): `langchain>=0.1.14`, `langchain-core>=0.1.22`, `langchain-community>=0.3.0,<0.4` (capped for Ragas 0.4.3's `vertexai` import), `langchain-openai>=0.0.8`, `langgraph>=0.0.15`, `langchain-neo4j>=0.1.0`. Floors are pre-1.0 — generations behind the 1.x line. The unused `langchain-groq` and direct `groq` pins were removed because Groq traffic belongs behind LiteLLM and no Backend module imported either package.
