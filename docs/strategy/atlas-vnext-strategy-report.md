...
Strong candidates intentionally below the top 20: `imgproxy` is small and useful but follows the dashboard/asset-browser work; its July 4, 2026 watchlist decision keeps it out of the service graph until Atlas has backend-generated signed URLs, MinIO bucket allow-lists, and a route/auth model for thumbnails. `NocoDB` is attractive for human-in-the-loop queues but waits on SSO; `NeoDash` waits until more services write useful graph data; `WhisperX` waits until meeting/audio ingestion becomes a first-class RAG flow; `Dagster` and `Superset` wait until the MinIO/Iceberg/Trino foundation has proven demand; `TimescaleDB` is best treated as part of the trading data slice rather than a standalone platform bet.
