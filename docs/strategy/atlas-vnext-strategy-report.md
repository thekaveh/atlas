...
Strong candidates intentionally below the top 20: `imgproxy` is small and useful but follows the dashboard/asset-browser work; its July 4, 2026 watchlist decision keeps it out of the service graph until Atlas has backend-generated signed URLs, MinIO bucket allow-lists, and a route/auth model for thumbnails. `NocoDB` is attractive for human-in-the-loop queues, but its July 4, 2026 watchlist decision keeps it out until Atlas has a concrete review workflow plus credible SSO/route auth. `NeoDash` waits until more services write useful namespaced graph data and its unsupported Labs image is acceptable for the selected use case; `WhisperX` waits until meeting/audio ingestion becomes a named RAG/voice workflow with diarization terms, resource posture, and provenance solved; `Dagster` and `Superset` wait until the MinIO/Iceberg/Trino foundation has proven demand; `TimescaleDB` is best treated as part of the trading data slice rather than a standalone platform bet.
...
- **Supabase Edge Functions:** defer because Atlas already has backend, n8n, Celery, and Airflow for server-side execution; a second Deno function surface adds overlap before there is an edge-specific need. The July 4, 2026 decision keeps Supabase Edge Functions deferred until Atlas has a concrete edge-specific use case and accepts the self-hosting beta/runtime-upgrade posture.
- **OpenLIT:** defer as a standalone candidate because Langfuse plus the OTel Collector/Tempo/Loki path already covers the first observability slice with less UI overlap. The July 4, 2026 decision keeps OpenLIT deferred until Atlas has a named GenAI telemetry gap and clear ClickHouse, collector, UI, retention, and auth ownership boundaries.
- **Live trading services:** defer Hummingbot, Freqtrade, and NautilusTrader live execution. Reconsider only after paper mode, secrets, audit logs, and explicit operator risk controls exist. The July 4, 2026 decision keeps live trading services rejected for now until Atlas can prove paper/sandbox-only service admission, secrets-manager-backed credentials, auditability, no-live-order route gating, and explicit human approval for any future promotion path.
- **FinRL and FinGPT:** keep in notebooks/research. Do not present them as production trading intelligence. The July 4, 2026 decision keeps FinRL and FinGPT deferred to notebooks until Atlas has curated datasets, evaluation criteria, paper-trading guardrails, model/resource limits, and clear "not financial advice" copy.
...
- **Hunyuan3D, TRELLIS/TRELLIS.2, Nerfstudio, Unreal MCP, and LiveKit:** watchlist for the 3D/game track, but not before the asset pipeline and MCP safety posture are real. The July 4, 2026 decision keeps heavy 3D/game infrastructure deferred until Atlas has asset storage, thumbnails, glTF validation, disabled-localhost editor automation, resource budgets, and no-Kong editor-control guarantees.
- **Voicebox, OmniVoice, and Unmute:** defer as voice-stack watchlist items; Voicebox lacks the OpenAI-compatible endpoint Atlas needs, OmniVoice would require Atlas to own a young HTTP wrapper, and Unmute is still too early for Atlas to standardize on as a core realtime speech layer. The July 4, 2026 decision keeps Voicebox, OmniVoice, and Unmute deferred until Atlas has a clearer realtime speech workflow, endpoint compatibility, no-wrapper-ownership path, voice-consent posture, and safe WebSocket/MCP route model.
- **Honcho:** defer because Atlas already has LangMem and Graphiti is a lighter first experiment; Honcho's AGPL posture and separate memory service are not yet justified. The July 4, 2026 decision keeps Honcho deferred until LangMem and Graphiti are proven insufficient for a named cross-agent memory workflow and Atlas has accepted AGPL, route-auth, tenant-isolation, deriver, and memory-deletion/export requirements.
- **Redis Stack and RedisInsight:** defer until a concrete Redis module or GUI workflow beats the license/image-size cost. The July 4, 2026 decision keeps Redis Stack and RedisInsight deferred until Atlas has a named module-backed or operator-GUI workflow, explicit Redis license/terms acceptance, route auth for RedisInsight, and module persistence/restore tests.
- **Perplexica/Vane:** defer because it overlaps with Open WebUI plus Local Deep Researcher unless a distinct "single-shot cited answer" product surface becomes a priority. The July 4, 2026 decision keeps Perplexica/Vane deferred until Atlas intentionally designs a separate cited-answer app with route auth, provenance, upload/storage, and clear differentiation from Open WebUI web search and Local Deep Researcher reports.
...
- `docs/operations/ports-and-routes.md`: authoritative Kong alias table, root-route behavior, and direct evidence of route sprawl.
...
- Atlas-internal evidence used for the Kong-root recommendation: [ports-and-routes](../operations/ports-and-routes.md), [Kong README](https://github.com/thekaveh/atlas/blob/main/services/kong/README.md), [Open WebUI README](https://github.com/thekaveh/atlas/blob/main/services/open-webui/README.md), [LiteLLM README](https://github.com/thekaveh/atlas/blob/main/services/litellm/README.md), [Hermes README](https://github.com/thekaveh/atlas/blob/main/services/hermes/README.md), and the current [ROADMAP](../ROADMAP.md).

Task 5 vNext ranking notes (candidate corpus plus current outside sources checked July 2, 2026):

- Existing one-pagers considered for ranking: [Apache Tika](../research/candidates/apache-tika.md), [Browserless](../research/candidates/browserless.md), [Celery + Flower](../research/candidates/celery-flower.md), [Crawl4AI](../research/candidates/crawl4ai.md), [Dagster](../research/candidates/dagster.md), [Docling MCP](../research/candidates/docling-mcp.md), [Firecrawl](../research/candidates/firecrawl.md), [Grafana Loki](../research/candidates/grafana-loki.md), [Graphiti](../research/candidates/graphiti.md), [Honcho](../research/candidates/honcho.md), [Iceberg + DuckDB](../research/candidates/iceberg-duckdb.md), [imgproxy](../research/candidates/imgproxy.md), [Keycloak](../research/candidates/keycloak.md), [Label Studio](../research/candidates/label-studio.md), [Lakekeeper](../research/candidates/lakekeeper.md), [Langfuse](../research/candidates/langfuse.md), [MCP Gateway](../research/candidates/mcp-gateway.md), [mcpo](../research/candidates/mcpo.md), [MLflow](../research/candidates/mlflow.md), [Neo4j LLM Knowledge Graph Builder](../research/candidates/neo4j-llm-graph-builder.md), [NeoDash](../research/candidates/neodash.md), [NocoDB](../research/candidates/nocodb.md), [OmniVoice](../research/candidates/omnivoice.md), [Open WebUI Pipelines](../research/candidates/open-webui-pipelines.md), [OpenLIT](../research/candidates/openlit.md), [Perplexica/Vane](../research/candidates/perplexica.md), [Prometheus](../research/candidates/prometheus.md), [Redis Stack](../research/candidates/redis-stack.md), [RedisInsight](../research/candidates/redisinsight.md), [SigLIP 2 Vectorizer](../research/candidates/siglip2-vectorizer.md), [Supabase Edge Functions](../research/candidates/supabase-edge-functions.md), [Supavisor](../research/candidates/supavisor.md), [Superset](../research/candidates/superset.md), [Unmute](../research/candidates/unmute.md), [Verba](../research/candidates/verba.md), [Voicebox](../research/candidates/voicebox.md), and [WhisperX](../research/candidates/whisperx.md).
...
| NocoDB | Watchlist | July 4, 2026 decision keeps it out of the service graph until Atlas has one concrete human-review queue and a credible SSO/route-auth posture; it must not become a second admin database UI. |
| NeoDash | Watchlist | July 4, 2026 decision keeps it out of the service graph until Atlas has richer namespaced Neo4j application data, a root-dashboard boundary, and an explicit answer to the unsupported Neo4j Labs image risk. |
| WhisperX | Watchlist | July 4, 2026 decision keeps it out of the service graph until meeting/audio ingestion is a named workflow with pyannote token/model terms, resource limits, and transcript provenance defined. |
| Dagster | Watchlist | July 4, 2026 decision keeps it out of the service graph until Atlas has one concrete asset-lineage workflow and an explicit Airflow coexistence or migration plan. |
