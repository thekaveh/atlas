...
- **Hosted-media provider contracts** — FAL configuration now fails closed on malformed booleans, non-finite or excessive timeouts, and unsupported output formats. The two default FLUX endpoints receive only schema-supported fields, custom endpoints require explicit provider-native arguments, FAL security/persistence documentation matches Redis-backed owner-scoped operation state, and the ineffective Parakeet GPU compute-type setting and `int8` guidance are removed.
