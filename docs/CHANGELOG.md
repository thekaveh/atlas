...
- **Isolated LightRAG parser adapter** — In-stack LightRAG v1.5.4 now reaches Docling through an unexposed compatibility adapter on `docling-lightrag-network`; LightRAG never receives `DOCLING_API_TOKEN`. The adapter implements `POST /v1/convert/file/async` (multipart field `files`), `GET /v1/status/poll/{task_id}`, `GET /v1/result/{task_id}`, and `GET /health`, with bounded jobs, attempts, streamed result size, and artifact TTL.
