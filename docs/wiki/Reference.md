...
| BACKEND_PLUGINS_SITE_DIR | backend | /tmp/atlas-plugins-site | Writable directory the plugin seam pip-installs plugin requirements into (`pip --target`) and prepends to sys.path before plugin imports (#559). The image runs as appuser with root-owned site-packages and no $HOME, so untargeted installs fail with EACCES. Must be creatable by the backend user; falls back to untargeted pip install when it cannot be created. |
...
| asset-baker | ASSET_BAKER_PORT | asset-baker.localhost |
...
| comfyui | COMFYUI_PORT, COMFYUI_LOCALHOST_PORT, COMFYUI_MPS_LOCALHOST_PORT | comfyui.localhost |
