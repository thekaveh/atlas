...
def test_backend_upload_and_cors_knobs_are_manifested_and_composed() -> None:
    ...
    assert env_vars["RAG_INGESTION_MAX_FILE_BYTES"]["default"] == 104857600
    assert env_vars["RAG_INGESTION_MAX_CORPUS_BYTES"]["default"] == 1073741824
    assert env_vars["RAG_INGESTION_MAX_FILES"]["default"] == 10000
    ...
    assert backend_env["RAG_INGESTION_MAX_FILE_BYTES"] == (
        "${RAG_INGESTION_MAX_FILE_BYTES:-104857600}"
    )
    assert backend_env["RAG_INGESTION_MAX_CORPUS_BYTES"] == (
        "${RAG_INGESTION_MAX_CORPUS_BYTES:-1073741824}"
    )
    assert backend_env["RAG_INGESTION_MAX_FILES"] == (
    ...
    assert "RAG_INGESTION_MAX_FILE_BYTES=104857600" in env_example
    assert "RAG_INGESTION_MAX_CORPUS_BYTES=1073741824" in env_example
    assert "RAG_INGESTION_MAX_FILES=10000" in env_example
