...
class RagCorpus:
    ...
    access_key_var: str | None = None  # generated from matching consumer storage
    secret_key_var: str | None = None  # generated from matching consumer storage
    ...
    def as_dict(self) -> dict[str, Any]:
        ...
        if self.source == "mount":
            ...
            if self.access_key_var and self.secret_key_var:
                out["access_key_var"] = self.access_key_var
                out["secret_key_var"] = self.secret_key_var
        return out
...
def render_n8n_seed_overlay(workflows: Iterable[N8nWorkflow]) -> str:
    ...
    lines = [
        ...
        "      DB_POSTGRESDB_USER: ${SUPAVISOR_DB_USER:-${N8N_DB_USER:?N8N_DB_USER is required}}",
...
def _bind_rag_minio_credentials(
    profiles: Iterable[RagIngestionProfile],
    stores: Iterable[StorageStore],
) -> list[RagIngestionProfile]:
    stores = list(stores)
    bound: list[RagIngestionProfile] = []
    for profile in profiles:
        if profile.corpus.source != "minio":
        ...
        store = next(
            (
                ...
                if candidate.consumer == profile.consumer
                and profile.corpus.bucket in candidate.all_buckets
            ),
            None,
        )
        if store is None:
            raise ConsumerManifestError(
                f"rag_ingestion_profiles[{profile.name!r}] MinIO bucket "
                f"{profile.corpus.bucket!r} must be declared under storage.buckets "
                f"by consumer {profile.consumer!r}"
            )
        corpus = replace(
            profile.corpus,
            access_key_var=store.access_var,
            secret_key_var=store.secret_var,
        )
        bound.append(replace(profile, corpus=corpus))
    return bound
...
def render_rag_ingestion_overlay(profiles: Iterable[RagIngestionProfile]) -> str:
    ...
    the API and worker and points ``RAG_INGESTION_PROFILES_FILE`` at it.
    ...
    credential_vars = sorted(
        {
            ...
            for profile in profiles
            for var in (
                profile.corpus.access_key_var,
                profile.corpus.secret_key_var,
            )
            if var
        }
    )
    lines = [
        ...
        "# Mounts the compiled rag_ingestion_profiles into the API and worker so",
        "# both execution paths resolve the same profile by name. Regenerated",
        "# every start; a removed manifest drops the file + this overlay.",
        "services:",
        "  backend:",
        "    environment:",
        f"      RAG_INGESTION_PROFILES_FILE: {RAG_INGESTION_CONTAINER_PATH}",
        *[f"      {var}: ${{{var}:-}}" for var in credential_vars],
        "    volumes:",
...
def load_consumer_config(
    ...
    *,
    ...
    rag_ingestion_overlay: GeneratedArtifact | None = None
    if all_rag:
        all_rag = _bind_rag_minio_credentials(all_rag, all_storage)
