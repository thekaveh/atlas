...
from the YAML catalogs (``services/ollama/models.yaml``,
``services/litellm/models.yaml``) + env vars via ``model_resolver`` on
every ``docker compose up`` — this file produces only a minimal stub on
the host so the bind mount
...
class LiteLLMConfigGenerator:
    ...
    # rendered from the YAML catalogs via model_resolver — leave it alone
    # so re-running ``./start.sh`` and quitting before docker compose up
    # doesn't blow away the user's previous active model_list.
    _LITELLM_INIT_SENTINEL = "# Generated by litellm-init/init.py from YAML catalogs via model_resolver."
    ...
    def write_config(self, output_path: Path, force: bool = False) -> bool:
        ...
        with open(output_path, "w", encoding="utf-8") as fh:
            ...
            fh.write("# The real model_list is rendered by litellm-init from YAML catalogs\n")
            fh.write("# (via model_resolver) on every `docker compose up`. Don't edit by\n")
            fh.write("# hand — change models via the wizard or env vars in .env.\n\n")
